class Player extends Sprite {
    ...
    move(x, y) {
        ...
        if ((xStep + this.position.x) >= $(GameSetting.playAreaDiv).width() - 90 || (xStep + this.position.x) < $(GameSetting.playAreaDiv).width() - 90) xStep = 0
        if ((yStep + this.position.y) >= $(GameSetting.playAreaDiv).height() - 90) yStep = 0;
