class Player extends Sprite {
    ...
    move(x, y) {
        ...
        alert(xStep + this.position.x)
        if ((xStep + this.position.x) >= $(GameSetting.playAreaDiv).width()) xStep = 0;
        if ((yStep + this.position.y) >= $(GameSetting.playAreaDiv).height()) yStep = 0;
